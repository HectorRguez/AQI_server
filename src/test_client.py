...
class AQITestClient:
    ...
    def __init__(self, base_url="http://localhost:5000"):
        ...
        # Setup figures directory at the same level as src and data
        self.figures_dir = os.path.join(os.path.dirname(__file__), '..', 'figures')
        os.makedirs(self.figures_dir, exist_ok=True)
        print(f"Figures will be saved to: {os.path.abspath(self.figures_dir)}")
    ...
    def test_forecast_aqi(self):
        ...
        for lat, lon, city in test_cities:
            ...
            response = self.session.get(f"{self.base_url}/api/forecast",
                                      params={"lat": lat, "lon": lon})
            ...
            if response.status_code == 200:
                ...
                if data.get('list'):
                    ...
                    # Get next 24 hours average AQI
                    next_24h = data['list'][:8]  # 3-hour intervals
    ...
    def test_historical_aqi(self):
        ...
        for lat, lon, city in test_cities:
            ...
            response = self.session.get(f"{self.base_url}/api/historical",
                                      params={
                                          "lat": lat,
                                          "lon": lon,
    ...
    def _plot_current_aqi(self, data, city_name):
        ...
        values = [
            ...
            components.get('co', 0) / 100,  # Scale CO for better visualization
        ...
        for bar, value, pollutant in zip(bars, values, pollutants):
            ...
            # Restore CO to original scale for label
            display_value = value * 100 if pollutant == 'CO' else value
            plt.text(bar.get_x() + bar.get_width()/2., height,
                    f'{display_value:.1f}', ha='center', va='bottom')
        
        plt.xlabel('Pollutants')
        plt.ylabel('Concentration (μg/m³)')
        plt.title(f'{city_name} - Current Air Quality (AQI: {data["list"][0]["main"]["aqi"]})')
        plt.tight_layout()
        
        # Save to figures directory
        filepath = os.path.join(self.figures_dir, f'current_{city_name.lower()}.png')
    ...
    def _plot_forecast_aqi(self, data, city_name):
        ...
        ax1.plot(timestamps, aqi_values, color=color, linewidth=2, marker='o', markersize=4)
        ...
        ax2.plot(timestamps, pm25_values, color=color, linewidth=2, linestyle='--', alpha=0.7)
        ...
        # Save to figures directory
        filepath = os.path.join(self.figures_dir, f'forecast_{city_name.lower()}.png')
        plt.savefig(filepath)
        plt.close()
        print(f"  Saved: {os.path.basename(filepath)}")
    ...
    def _plot_historical_aqi(self, data, city_name):
        ...
        # Save to figures directory
        filepath = os.path.join(self.figures_dir, f'historical_{city_name.lower()}.png')
        plt.savefig(filepath)
        plt.close()
        print(f"  Saved: {os.path.basename(filepath)}")
    ...
    def run_all_tests(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"  Historical data: {health_data.get('historical_days', 'N/A')} days")
                print(f"  Database: {os.path.basename(health_data.get('database', 'N/A'))}")
            else:
        ...
        try:
            ...
            if response.status_code == 200:
                info = response.json()
                print(f"  Update interval: {info.get('update_interval_seconds', 'N/A')} seconds")
        except:
            pass
        ...
        print(f"Generated plots saved to: {os.path.abspath(self.figures_dir)}")
